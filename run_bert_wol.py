...
flags.DEFINE_string("tag_file", "./data/clue_ner/tag2label.txt",
                    "The label set of ner dataset.")

flags.DEFINE_string(
    "output_dir", "D:/Expriment/model_output/ner_tool/bert_wol/single_task/clue_ner/runs/checkpoints",
...
flags.DEFINE_bool("use_weights_of_loss", True, "Whether to run training.")
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        return self._read_label(FLAGS.tag_file)
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, label2id, bmeo_labels,
  ...
  bmeo_o_index = label2id["O"]
  vocab_size_bmeo = len(label2id)
  ...
  with tf.variable_scope('loss'):
      if not FLAGS.use_crf:
          # logits_bmeo 是预测结果，形状为 B*S*V，softmax 之后就是每个字在BMEO词表上的分布概率，不过不用写softmax，因为下面的函数会帮你做
          # bmeo_labels 是期望输出，形状为 B*S
          # 这是原本计算出来的 loss
          loss_bmeo = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits_bmeo,
                                                                     labels=bmeo_labels)  # B * S
          if FLAGS.use_weights_of_loss:
              # 这是根据期望的输出，获得 mask 向量，向量里出现1的位置代表对应的字是一个实体词，而 O_tag_index 就是 O 在 BIO 词表中的位置
              masks_of_entity = tf.cast(tf.not_equal(bmeo_labels, bmeo_o_index), tf.float32)  # B * S
              # 这是基于 mask 计算 weights
              weights_of_loss = masks_of_entity + 0.5  # B * S
              # 这是加权后的 loss
              loss_bmeo = tf.reduce_sum(loss_bmeo * weights_of_loss, axis=-1) / (tf.reduce_sum(masks_of_entity, axis=-1) + 1e-5)  # B
          else:
      ...
      loss = tf.reduce_mean(loss_bmeo)

  return (loss, preds_bmeo)


def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
                     num_train_steps, num_warmup_steps, use_tpu,
                     use_one_hot_embeddings, id2char):
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    with open(os.path.join(FLAGS.output_dir, 'label2id.pkl'), 'rb') as f:
    ...
    (total_loss, pred_ids) = create_model(
        bert_config, is_training, input_ids, input_mask, segment_ids, bmeo_label2id, label_ids,
        use_one_hot_embeddings)
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def metric_fn(label_ids, pred_ids):
        ...
        for k, v in label2id.items():
            id2tag[v] = k
            if k == 'O' or k == '[CLS]' or k == '[SEP]' or k == 'X':
                pass
            else:
